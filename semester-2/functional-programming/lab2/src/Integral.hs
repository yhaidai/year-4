...
calculate x1 x2 eps numOfThreads = calculate_ x1 x2 (eps / fromIntegral numOfThreads) numOfThreads ((x2 - x1) / fromIntegral numOfThreads)

calculate_ :: Double -> Double -> Double -> Int -> Double ->(Double->Double)-> Double
calculate_ x1 x2 eps numOfThreads step f =
  let trap_squares =
        [ integrate_part (x1 + (step * fromIntegral i)) (x1 + (step * (fromIntegral i + 1))) step eps
          (trapSquaresSum (x1 + (step * fromIntegral i)) (x1 + (step * (fromIntegral i + 1))) step f)
          (trapSquaresSum (x1 + (step * fromIntegral i)) (x1 + (step * (fromIntegral i + 1))) (step / 2.0) f)
...
integrate_part x1 x2 step eps previous current f
    | checkIfStepIsCorrect previous current eps = current
    | otherwise = integrate_part x1 x2 (step / 2.0) eps (trapSquaresSum x1 x2 (step / 2.0) f) (trapSquaresSum x1 x2 (step / 4.0) f) f
